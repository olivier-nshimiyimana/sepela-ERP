...
export default function ExpiryAlertsBanner({ products, expiryAlertDays, onManageProducts }) {
  ...
  if (dismissed) {
    return (
      <button
        ...
        className="fixed top-16 right-3 z-40 flex items-center gap-1.5 px-2.5 py-1.5 rounded-full border border-amber-800/80 bg-[#1a1a1a]/95 text-amber-400 shadow-lg backdrop-blur-sm hover:bg-amber-950/60"
  ...
  return (
    <Box
      className="fixed top-16 right-3 z-40 w-72 max-w-[calc(100vw-1.5rem)] rounded-lg border border-amber-900/60 bg-[#1a1a1a]/95 shadow-xl backdrop-blur-sm overflow-hidden"
      role="status"
      aria-live="polite"
    >
      <Box className="flex items-start gap-2 px-3 py-2">
        ...
        <Box className="flex-1 min-w-0">
          ...
          <p className="text-[10px] text-gray-400 mt-0.5 truncate">{summaryParts.join(" · ")}</p>
        </Box>
        <Box className="flex items-center gap-0.5 shrink-0">
          <button
            type="button"
            onClick={() => setExpanded((open) => !open)}
            className="p-1 rounded text-gray-500 hover:text-amber-300"
      ...
      {expanded ? (
        <Box className="border-t border-amber-900/40 px-3 py-2 max-h-36 overflow-y-auto space-y-1.5">
          ...
          <button
            ...
            className="w-full mt-1 py-1.5 text-[10px] font-bold uppercase text-amber-400 border border-amber-900/50 rounded hover:bg-amber-950/40"
          >
        ...
        <Box className="px-3 pb-2 flex gap-2">
          <button
            ...
            className="flex-1 py-1 text-[10px] font-bold uppercase text-gray-400 hover:text-amber-300"
          >
            {t("common.details")}
          </button>
          <button
            type="button"
            onClick={onManageProducts}
            className="flex-1 py-1 text-[10px] font-bold uppercase text-amber-400 border border-amber-900/50 rounded hover:bg-amber-950/40"
          >
            {t("expiry.review")}
          </button>
        </Box>
